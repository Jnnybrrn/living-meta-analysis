...
function requestAndFillMetaanalysis() {
  ...
  function fillMetaanalysis(metaanalysis) {
    ...
    function loadAllMetaanalysisPapers(metaanalysis) {
      fetch(currentMetaanalysisUrl+'/papers/')
      .then(_.fetchJson)
      .then(function (papers) {
        metaanalysis.papers = papers;
        metaanalysis.paperOrder = [];
        for (var i = 0; i < papers.length; i++){
          metaanalysis.paperOrder.push(papers[i].id);
        }
      })
      .catch(function (err) {
        console.error('problem getting metaanalysis papers');
        console.error(err);
      });
    }

    loadAllMetaanalysisPapers(metaanalysis);
    console.log(metaanalysis);
    // Remove this when we can add a paper.
    //metaanalysis.paperOrder = ["/id/p/1476385486001", "/id/p/1476279383995"];
  ...
  function fillMetaanalysisExperimentTable(metaanalysis) {
    console.log(metaanalysis.papers);
    var papers = metaanalysis.papers;
    console.log("papers translates to.. ", papers);
